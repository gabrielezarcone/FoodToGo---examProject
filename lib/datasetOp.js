...
function info_piatto(piatto,piatto_node){
		...
		var addToChart = document.createElement('div');
		//-------------------------------------
		...
		addToChart.setAttribute('onclick', 'addToChart("'+piatto+'");')
		addToChart.className = 'button addToChart_btn';
		//-------------------------------------
		...
		addToChart.innerText = 'Aggiungi al carrello';
		//-------------------------------------
		...
		if(user_type=='clienti'){
			infoBox.appendChild(addToChart);
		}
	...
	for(input of form){
		if(input.name == 'preferences' ){
			if( input.checked == true){
				...
				console.log('new: ' + new_user[input.name]);
			}
		}
...
function addToChart(piatto){
	var user = username();
	var clienti = db('clienti');
	clienti[user]['carrello'] = clienti[user]['carrello'].concat(piatto);
