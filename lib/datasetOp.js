...
function update_user_info(){
		...
		for(field of uff){
				if(field.name!=''){	
						...
						console.log(clienti[user][field.name]);
...
function addPiatto(){
	var piatti_window = open_float_window();
	var piatti = db('piatti');
	//------------------------------------------------
	var form = document.createElement('form');
	form.innerHTML = '<input list="piatti_datalist">';
	//------------------------------------------------
	...
	//------------------------------------------------
	piatti_window.appendChild(form);
}
