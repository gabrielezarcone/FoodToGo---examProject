		...
		for(field of uff){
				if(field.name!=''){	
						if(field.name == 'preferences'){
							clienti[user][field.name] = field.value.split(',');
						}
...
function type_choosen(){
	...
	var preferences = document.createElement('input');
...
function createUser(user_type){
	...
	for(input of form){
		if(input.name == 'preferences'){
			new_user[input.name] = input.value.split(',');
		}
	...
	//---------------------------------------------------
	users[form['username'].value] = new_user;
	update_db(user_type, users);
}

function show_preferences_list(){
	...
	//---------------------------------------------------
	options.setAttribute('onchange', 'choose_piatto()');
	//---------------------------------------------------
	var info = document.createElement('option');
	info.innerText = 'Seleziona i tuoi piatti preferiti';
	info.disabled = true;
	info.selected = true;
...
function choose_piatto(){
	var selector = event.currentTarget;
	var input = document.getElementById('pref_input');
	var preferences = input.value;
	var new_pref = preferences + selector.value + ',';
	input.value = new_pref;
	input.placeholder = new_pref;
}